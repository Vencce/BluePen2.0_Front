...
const carregarMovimentos = async () => {
  ...
  try {
    const response = await axios.get(
      'https://bluepen-back.onrender.com/api/fabrica/movimentos-produto/',
    ...
    console.error('Erro ao carregar movimentos de produtos:', error)
...
<template>
  <AdminLayout>
      ...
      <div class="page-header-flex">
        <h1>Relatório de Movimentação de Produtos</h1>
      </div>
              ...
              <tbody>
                <tr v-for="m in movimentos" :key="m.id">
                  ...
                  <td :class="[m.tipo === 'ENTRADA' ? 'text-success' : 'text-danger']">
                    {{ formatCurrency(m.custo_producao_unitario) }}
                  </td>
